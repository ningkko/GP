...
(defn get-input
  ...
  [filename]
  (let [raw-input #(drop-last (rest (read-row filename %)))]
    (map #(concat (apply #(map to-float %) (take 4 (apply (raw-input %))))
                  (boolean (first (take-last 7 (apply (raw-input %)))))
                  (apply #(map to-float %) (rest (take-last 7 (apply (raw-input %))))))
